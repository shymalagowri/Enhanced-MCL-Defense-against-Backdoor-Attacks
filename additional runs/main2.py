...
def test(opt, test_clean_loader, test_bad_loader, nets, criterions, epoch):
    ...
    # Calculate and print confusion matrix for clean images
    cm_clean = confusion_matrix(y_true_clean, y_pred_clean)
    print("Confusion Matrix for Clean Images:")
    print(cm_clean)

    # Calculate and print confusion matrix for bad images
    cm_bad = confusion_matrix(y_true_bad, y_pred_bad)
    print("Confusion Matrix for Bad Images:")
    print(cm_bad)

    # Plot confusion matrix for clean images
    plt.figure(figsize=(8, 6))
    sns.heatmap(cm_clean, annot=True, fmt="d", cmap="Blues", xticklabels=np.unique(y_true_clean),
                yticklabels=np.unique(y_true_clean))
    plt.title("Confusion Matrix for Clean Images")
    plt.xlabel("Predicted Label")
    plt.ylabel("True Label")
    plt.show()

    # Plot confusion matrix for bad images
    plt.figure(figsize=(8, 6))
    sns.heatmap(cm_bad, annot=True, fmt="d", cmap="Reds", xticklabels=np.unique(y_true_bad),
                yticklabels=np.unique(y_true_bad))
    plt.title("Confusion Matrix for Bad Images")
    plt.xlabel("Predicted Label")
    plt.ylabel("True Label")
    plt.show()
...
if __name__ == '__main__':
    ...
    #print('op rev eng'+  reverse_engineer(opt) )
    print("end of main")
